...
export default function WorkspacePage() {
  ...
  const isWorkstationReady = lifecycleState === "ready" && currentDocument
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground antialiased selection:bg-accent selection:text-accent-foreground">
      ...
      >
        <div
          className={`${
            isWorkstationReady
              ? "max-w-[1700px] w-full mx-auto px-3 sm:px-5 lg:px-6"
              : "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"
          }`}
        >
          {/* Intake Context Bar (Only shown during intake/idle/processing) */}
          {!isWorkstationReady && (
            <div className="flex flex-wrap items-center justify-between gap-3 pb-6 mb-6 border-b border-border/60">
              <div className="flex items-center gap-2 text-xs text-ink-muted">
                ...
                {pendingDoc && (
                  <>
                    <span>/</span>
                    <span className="font-mono text-ink-secondary truncate max-w-xs" title={pendingDoc.name}>
                      {pendingDoc.name}
                    </span>
                  </>
                )}
